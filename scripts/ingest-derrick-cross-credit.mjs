/**
 * Idempotent legacy ingest for Derrick Cross / CL001122 (pet Jinju).
 * Source: Tax Invoice jinju cross April 20 2026 CREDIT from MSH.xlsx
 *
 * Usage: node scripts/ingest-derrick-cross-credit.mjs
 *        node scripts/ingest-derrick-cross-credit.mjs --dry-run
 */
...
const XLSX_PATH =
  "/Users/nawalhilal/Downloads/Tax Invoice jinju cross April 20 2026 CREDIT from MSH.xlsx";

const OWNER_ID = "5dbfd106-6897-4c02-92b7-b8caa45ff25e";
const PET_JINJU = "2c8819f6-b984-4d47-a660-91f09c07eab1";
const INGEST_KEY = "LEGACY:MSH-CROSS-APR20";
const EXPECTED_BALANCE = 3044.9;
...
async function ensureDaycareSession({ sessionDate, description, invoiceId, hourlyBilling }) {
...
async function markSessionInvoiced(sessionId, invoiceId) {
  ...
  if (!base.some((line) => line.startsWith("BILLING_PATH:"))) {
    base.push("BILLING_PATH:hourly");
  }
...
async function main() {
  ...
  for (const line of lines) {
    ...
    if (/sspl/i.test(service)) {
      const invoiceNumber = `MSH-CROSS-SSPL-${sessionDate}`;
    ...
    if (/nails/i.test(service)) {
      const invoiceNumber = `MSH-CROSS-NAILS-${invoiceSlug(service)}`;
      let invoiceId = (await findExistingInvoice(invoiceNumber))?.id;
      if (!invoiceId) {
        invoiceId = await createInvoice({
          invoiceNumber,
    ...
    if (/dcare/i.test(service)) {
      ...
      const invoiceNumber = `MSH-CROSS-DC-${invoiceSlug(service)}`;
      let sessionId = (await findExistingDaycareSession(sessionDate, marker))?.id;
      if (!sessionId) {
        sessionId = await ensureDaycareSession({
          sessionDate,
          description: service,
          invoiceId: null,
          hourlyBilling: true,
      ...
      if (!invoiceId) {
        invoiceId = await createInvoice({
          ...
          lineDescription: `Daycare hourly — Jinju (${hours} hr @ AED ${HOURLY_UNIT_RATE}/hr, 20% discount)`,
          quantity: qty,
          unitPrice: round2(amount / qty),
