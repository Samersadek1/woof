-- Custom daycare packages: optional manual invoice line items (multi-line pricing)
-- and 10% multi-pet discount (same rule as catalog packages).

UPDATE package_definitions
SET multi_pet_discount_pct = 10
WHERE code = 'custom_daycare';

-- 10-arg implementation (adds p_line_items jsonb)
...
DECLARE
  ...
  v_expires date;
  v_use_custom_lines boolean := false;
  v_line record;
...
BEGIN
  ...
  v_use_custom_lines := p_line_items IS NOT NULL AND jsonb_typeof(p_line_items) = 'array' AND jsonb_array_length(p_line_items) > 0;

  IF v_use_custom_lines THEN
    FOR v_line IN
      SELECT *
    ...
    LOOP
      ...
      IF v_line_desc_in IS NULL THEN
        RAISE EXCEPTION 'Each line item requires a description';
      END IF;
  ...
  IF v_use_custom_lines THEN
    ...
    LOOP
      ...
      INSERT INTO invoice_line_items (
        ...
        v_invoice_id, v_line_desc_in, v_line_qty, v_line_unit, v_line_total, v_line_total, 'package'
      );
...
-- 9-arg wrapper (backward compatible)
CREATE OR REPLACE FUNCTION public.issue_custom_daycare_package(
  p_owner_id uuid,
  p_pet_ids uuid[],
  p_units integer,
...
-- 8-arg wrapper (backward compatible)
CREATE OR REPLACE FUNCTION public.issue_custom_daycare_package(
  p_owner_id uuid,
  p_pet_ids uuid[],
  p_units integer,
...
REVOKE ALL ON FUNCTION public.issue_custom_daycare_package(uuid, uuid[], integer, numeric, text, integer, payment_method, service_code, date, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.issue_custom_daycare_package(uuid, uuid[], integer, numeric, text, integer, payment_method, service_code, date, jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.issue_custom_daycare_package(uuid, uuid[], integer, numeric, text, integer, payment_method, service_code, date, jsonb) TO service_role;
