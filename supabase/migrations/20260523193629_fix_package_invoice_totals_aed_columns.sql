...
BEGIN
  ...
  INSERT INTO invoices (
    owner_id, issue_date, status, subtotal, subtotal_aed, discount_amount, discount_aed, total, total_aed, vat_aed, payment_method, service_type
  ) VALUES (
    p_owner_id, CURRENT_DATE, 'issued'::invoice_status, 0, 0, 0, 0, 0, 0, 0, p_payment_method, 'package'
  )
  ...
  SET subtotal = v_subtotal,
      ...
      total = v_total,
      total_aed = v_total,
      vat_aed = ROUND(v_total - (v_total / 1.05), 2),
      updated_at = now()
...
SET subtotal_aed = CASE
      WHEN coalesce(subtotal_aed, 0) = 0 AND coalesce(subtotal, 0) > 0 THEN subtotal
      ELSE subtotal_aed
    END,
    discount_aed = CASE
      WHEN coalesce(discount_aed, 0) = 0 AND coalesce(discount_amount, 0) > 0 THEN discount_amount
      ELSE discount_aed
    END,
    discount_pct = CASE
      WHEN coalesce(subtotal, 0) > 0 THEN ROUND((coalesce(discount_amount, 0) / subtotal) * 100, 2)
      ELSE discount_pct
    END,
    total_aed = CASE
      WHEN coalesce(total_aed, 0) = 0 AND coalesce(total, 0) > 0 THEN total
      ELSE total_aed
    END,
    vat_aed = CASE
      WHEN coalesce(vat_aed, 0) = 0 AND coalesce(total_aed, 0) > 0
        THEN ROUND(total_aed - (total_aed / 1.05), 2)
      WHEN vat_aed IS NULL AND coalesce(total, 0) > 0
        THEN ROUND(total - (total / 1.05), 2)
      ELSE vat_aed
    END
WHERE service_type = 'package'
  AND (
    (coalesce(total_aed, 0) = 0 AND coalesce(total, 0) > 0)
    OR (coalesce(subtotal_aed, 0) = 0 AND coalesce(subtotal, 0) > 0)
    OR (coalesce(discount_aed, 0) = 0 AND coalesce(discount_amount, 0) > 0)
    OR vat_aed IS NULL
  );
