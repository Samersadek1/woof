-- Include draft-only owners on the payments triage list (active accruals not yet due).
...
-- SELECT owner_name, due_now, in_progress, total_balance
-- FROM get_owners_with_collectable_debt()
-- WHERE due_now = 0 AND in_progress > 0;
-- SELECT COUNT(*), SUM(due_now), SUM(in_progress), SUM(total_balance)
-- FROM get_owners_with_collectable_debt();
