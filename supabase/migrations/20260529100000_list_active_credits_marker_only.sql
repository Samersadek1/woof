-- Shared daycare pools are driven ONLY by the explicit invoice marker
-- (shared_pool_30_combined), not by purchase_groups.pet_count. Multi-pet
-- purchases otherwise keep per-dog credits with their own day caps.
      ...
      OR (
        EXISTS (
          ...
          WHERE inv.id = pg.invoice_id
            AND inv.notes ILIKE '%shared_pool_30_combined%'
        )
        AND EXISTS (
          SELECT 1
