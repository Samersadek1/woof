...
-- Does NOT remove park_lane room wing/type (dog kennel "Park Lane" suites).

-- ── Rooms: deactivate legacy cattery wing ─────────────────────────────────────
UPDATE public.rooms
SET is_active = false,
    updated_at = NOW()
WHERE wing::text = 'cattery';

-- ── Pricing: deactivate park rate-card service codes (if any were created) ───
...
-- ── Dashboard metrics: drop cattery + park_bookings from JSON payload ─────────
...
-- Verification (paste results after running):
...
-- SELECT public.get_dashboard_metrics(CURRENT_DATE) -> 'today' AS today;
