-- Neutralize SOA debt from status=paid + amount_paid=0 (no invoice_payments).
-- Root cause: phase 4b package import set status from tracker paid amount without
-- writing invoices.amount_paid. Does NOT change invoice rows — credits the ledger
-- with a review-flagged synthetic settlement so K is not inflated.
...
-- Verification
-- SELECT notes FROM get_ledger_statement('<owner>'::uuid, '2020-01-01+00'::timestamptz, now())
-- WHERE notes LIKE 'PAID_WITHOUT_SETTLEMENT%' LIMIT 5;
