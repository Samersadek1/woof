-- Short-window duplicate guard on invoice_payments.
-- Rejects identical invoice_id + amount + payment_method within 5 seconds
-- unless the insert goes through insert_invoice_payment(..., p_confirm_duplicate := true),
-- which sets a transaction-local GUC the BEFORE INSERT trigger honors.
-- Advisory xact lock serializes concurrent inserts per invoice so double-clicks
-- cannot both pass the EXISTS check.
...
BEGIN
  ...
  -- Serialize concurrent inserts for this invoice within the transaction.
...
-- SELECT proname FROM pg_proc WHERE proname = 'insert_invoice_payment';
