-- Daycare and package catalog prices are VAT-inclusive.
-- Legacy rows with vat_aed NULL were displayed/charged as ex-VAT (+5% on top).
-- This backfills vat_aed and aligns wallet/statement RPCs with gross-inclusive semantics.

BEGIN;

-- 1) Persist explicit VAT on package/daycare invoices (incl. zero-total legacy packages)
...
-- 2) Wallet payment: do not add 5% on package/daycare (or legacy daycare package notes)
...
-- 3) Statement of account: gross for package/daycare
...
-- 4) Cancellation refund basis: same gross rules
...
SELECT
  COUNT(*) FILTER (WHERE service_type IN ('package', 'daycare') AND vat_aed IS NULL) AS pkg_daycare_null_vat,
  COUNT(*) FILTER (
    WHERE service_type IN ('package', 'daycare')
      AND COALESCE(total_aed, 0) > 0
      AND ABS(
        COALESCE(vat_aed, 0)
        - ROUND(COALESCE(total_aed, 0) - COALESCE(total_aed, 0) / 1.05, 2)
      ) > 0.02
  ) AS pkg_daycare_vat_mismatch
