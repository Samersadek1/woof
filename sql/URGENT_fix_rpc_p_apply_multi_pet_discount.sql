-- URGENT: add p_apply_multi_pet_discount (boolean DEFAULT false) to
-- purchase_package and issue_custom_daycare_package so PostgREST accepts the
-- frontend opt-in discount flag. Supersedes 20260807151000 defaults (true -> false).
...
CREATE OR REPLACE FUNCTION public.purchase_package(
  ...
  p_apply_multi_pet_discount boolean DEFAULT false
)
RETURNS TABLE(
  invoice_id uuid,
...
DECLARE
  ...
  v_apply_discount boolean := coalesce(p_apply_multi_pet_discount, false);
BEGIN
...
-- 4-arg wrapper (no flag → opt-in default false)
CREATE OR REPLACE FUNCTION public.purchase_package(
  p_owner_id uuid,
  p_package_code text,
  p_pet_ids uuid[],
...
-- 10-arg wrapper (line items, no explicit flag → DEFAULT false)
CREATE OR REPLACE FUNCTION public.issue_custom_daycare_package(
  p_owner_id uuid,
  p_pet_ids uuid[],
  p_units integer,
...
-- Reload PostgREST schema cache (required for RPC arg discovery)
